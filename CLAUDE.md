...
- **Cirque chip scan**: independent of the MCU's state and of touch/glide activity — without chip-level power management the Pinnacle scans at its native ~100Hz continuously the whole time the MCU is awake, whether or not you're touching it. `CONFIG_ZMK_INPUT_PINNACLE_IDLE_SLEEPER=y` + `sleep;` on the `glidepoint` node (`toucan_right.overlay`) puts the chip itself to sleep after ~5s with no touches instead of scanning the whole 5-minute MCU-idle window. Costs ~300ms wake latency on the next touch. A prior tuning left this off, judging the latency not worth it when the MCU idle window was 1 hour; re-enabled now that the window is 5 min.
