...
- **Cirque chip scan**: without chip-level power management the Pinnacle scans at its native ~100Hz continuously any time the MCU is awake, whether or not you're touching it — datasheet-rated ~2.9mA active vs ~40µA asleep, so this can dominate battery life on its own. `CONFIG_ZMK_INPUT_PINNACLE_IDLE_SLEEPER=y` + `sleep;` on the `glidepoint` node (`toucan_right.overlay`) puts the chip to sleep. It is NOT on its own timer: `zmk_pinnacle_idle_sleeper.c` (in `cirque-input-module`) hooks `zmk_activity_state_changed` and sleeps the chip whenever ZMK leaves `ZMK_ACTIVITY_ACTIVE` — so it follows `CONFIG_ZMK_IDLE_TIMEOUT` (currently 30s), same knob as the rest of ZMK's idle state, not a separate 5s constant. Costs ~300ms wake latency on the next touch. A prior tuning left this off, judging the latency not worth it when the MCU idle window was 1 hour; re-enabled now that the window is 5 min.
